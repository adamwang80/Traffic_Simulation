...
def generate_cars(number_of_cars, p_res, p_biz):
    ...
    for _ in range(number_of_cars):
        # start_point = random.choice(list(road_graph.keys()))
        # end_point = random.choice(list(road_graph.keys()))
...
def simulation():
    ...
    p_res, p_biz = p_residential_business(numbers_exponential(total_time_steps))
    totals = np.array([])    #new from hana and zefang

    for step in range(total_time_steps):
        print(f"\nStep {step + 1}:")
        
        # Generate 1000 cars
        generate_cars(100, p_res[step], p_biz[step])
